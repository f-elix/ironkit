<script lang="ts">
	...
	let sortedKgPlateConfiguration = $derived(
		kgPlateConfiguration.toSorted((a, b) => b.plate - a.plate)
	);
</script>
...
<div class="mx-auto grid w-52 grid-stack">
	...
	<div class="flex items-center gap-1 pl-2">
		{#each sortedKgPlateConfiguration as { id, plate, count }, i (id)}
			{#if plate !== 0}
				{#each { length: count }}
					<div
						...
						class={[
							'rounded-lg',
							plate === 25 && 'h-32 w-3 bg-kg-plate-red',
							plate === 20 && 'h-32 w-3 bg-kg-plate-blue',
							plate === 15 && 'h-28 w-3 bg-kg-plate-yellow',
							plate === 10 && 'h-24 w-3 bg-kg-plate-green',
							plate === 5 && 'h-20 w-3 bg-kg-plate-white',
							plate === 2.5 && 'h-16 w-3 bg-kg-plate-black',
							plate === 1.25 && 'h-12 w-2 bg-kg-plate-silver',
							plate === 0.5 && 'h-8 w-1 bg-kg-plate-silver',
							plate === 0.25 && 'h-6 w-1 bg-kg-plate-silver'
						]}
					></div>
				{/each}
			{/if}
