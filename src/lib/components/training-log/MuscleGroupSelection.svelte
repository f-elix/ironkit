...
<fieldset class="space-y-2">
	...
	<ul class="flex flex-wrap gap-1">
		{#each value ?? [] as muscleGroup (muscleGroup)}
			...
			>
				<Badge variant="secondary" class="gap-2 rounded-full p-0.5 pl-2 font-normal">
					{label}
					<button
						type="button"
						class="bg-foreground grid size-6 place-items-center rounded-full"
						onclick={() => {
							value = value?.filter((mg) => mg !== muscleGroup);
						}}
						aria-label="Remove muscle group"
					>
						<X class="text-background size-4" />
					</button>
				</Badge>
			</li>
		{/each}
