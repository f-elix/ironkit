...
{#if workouts?.length}
	...
	>
		<Label class="flex flex-col gap-2">
			...
			<div class="relative">
				<Combobox.Input
					...
					class="border-border-input placeholder:text-foreground-alt/50 inline-flex h-10 w-full truncate rounded-sm border bg-background px-2 text-base transition-colors focus:outline-hidden focus:ring-2 focus:ring-foreground focus:ring-offset-2 focus:ring-offset-background sm:text-sm"
		...
		<Combobox.Portal>
			<Combobox.Content
				class="z-50 max-h-(--bits-combobox-content-available-height) w-(--bits-combobox-anchor-width) min-w-(--bits-combobox-anchor-width) select-none rounded-xl border border-muted bg-background px-1 py-3 shadow-popover outline-hidden data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2"
				sideOffset={10}
			>
				...
				<Combobox.Viewport class="p-1">
					{#each filteredItems as item, i (i + item.value)}
						<Combobox.Item
							class="flex h-10 w-full select-none items-center rounded-sm py-3 pl-5 pr-1.5 text-sm capitalize outline-hidden data-highlighted:bg-muted"
							value={item.value}
							label={item.label}
						>
