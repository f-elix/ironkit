<script lang="ts">
	...
	import { Performance, Workout } from '$lib/jazz/schema';
	...
	const workoutState = new CoState(
		Workout,
		() => (performance?.workoutId ? performance.workoutId : undefined)
	);
	const workout = $derived(workoutState.current.$isLoaded ? workoutState.current : null);
</script>
	...
	<ul class="flex flex-col gap-2">
		{#each sets as set, i (set.$jazz.id)}
			<li class="flex w-full items-baseline gap-2">
				<PerformanceSetSummary
					{set}
					{performance}
					order={i + 1}
					bodyweight={workout?.bodyweight ?? 0}
					bodyweightUnit={workout?.bodyweightUnit ?? 'lbs'}
				/>
			</li>
		{/each}
	</ul>
